...
export default function WorkerLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { name: "Overview", href: "/worker/dashboard", icon: LayoutDashboard },
    { name: "Profile & Settings", href: "/worker/settings/profile", icon: Settings },
  ];
    ...
    <div className="flex min-h-screen bg-gray-50 flex-col md:flex-row">
      <aside className="w-full md:w-64 border-r border-gray-200 bg-white flex flex-col md:flex-shrink-0">
        <div className="p-6 border-b border-gray-100 hidden md:block">
          <div className="font-bold text-lg text-gray-900">Worker Portal</div>
        </div>
